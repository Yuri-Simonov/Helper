...
3</code></pre>
            <p>
                В данном примере следует помнить о том, что <span class="attention">
					перед тем, как выполнять каждую следующую макрозадачу,
					<code>event loop</code> проверяет по цепочке приоритетов нет ли в данный момент синхронного кода и
					микрозадач, которые нужно выполнить</span>. Если нет, тогда он выполняет макрозадачу.
            </p>`,
    selected: false,
